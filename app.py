...
# ==============================================================================
            with dash_right:
                st.markdown("### ðŸ“Š Value Analytics & Tickets")
                ...
                st.info(f"ðŸ“Š Active Bookmaker Margin Audit: This market features a built-in **{computed_juice_percentage_tax:.1f}% Juice Tax**.")
...
st.dataframe(
    ...
    hide_index=True
        ...
        # ==============================================================================
        st.markdown("---")
        st.subheader("ðŸ† Bookmaker Outright Odds vs Monte Carlo Forecasts")
        ...
        for team in sorted(all_teams_raw):
            ...
            # ðŸŸ¢ THE REAL-TIME SYNC: If this row matches your selected team, read your screen entry!
            ...
            outright_rendered_payload.append({
                ...
                "Trading Outright Verdict": "ðŸ”¥ FUTURES ALPHA" if outright_expected_value >= 0.05 else ("ðŸ›‘ TRAP / FADE" if outright_expected_value <= -0.05 else "ðŸ”· EFFICIENT HOLD")
            })
        ...
        # ðŸ“Š THE ALGORITHMIC ALPHA SPOTLIGHT PANEL
        ...
        if net_ev_percentage >= 5.0:
            st.success(f"ðŸŽ¯ OUTRIGHT VALUE ALPHA DETECTED: Your Monte Carlo engine shows a clear +{net_ev_percentage:.1f}% Expected Value edge over the house for {target_audit_team}! This line is under priced.")
        elif net_ev_percentage <= -5.0:
            st.error(f"ðŸ›‘ MARKET VALUE TRAP: Retail books are heavily overpricing {target_audit_team} by {abs(net_ev_percentage):.1f}%. Avoid outright futures exposure.")
        else:
            st.info(f"ðŸ”· MARKET EFFICIENCY IN LINE: Sportsbook pricing for {target_audit_team} matches your 10,000-iteration lookback matrix within a tight {abs(net_ev_percentage):.1f}% error margin.")
